...
Shape* ShapeList::findClosestTo(double p_x, double p_y) const{
    //Set counter for smallest
    int counter = 0;
    //Set smallest value to first item
    double smallest = 0;
    ...
    for (const auto& item : *this){
        ...
        if (distance < smallest || counter == 0){
            smallest = distance;
            smallestShape = item;
        }
        counter++;
...
/**
 * @brief       Writes all points within the list
 *
 * @detailed    As the loop increments, it will call the
 *              write function until it is at the end of the list
 *              
 * @param       double p_x          x coordinate of user provided coordinate 
 *                                  pair
 * 
...
void ShapeList::print_shapes_containing(double p_x, double p_y) const{
    // for each `Shape*` in the list, if (p_x, p_y) is contained within the
    // Shape, write it to `cout`, one listing per line.
    for (const auto& shape : *this){       
        if (shape->contains(p_x, p_y)){ 
            shape->write(cout);             
            cout << "\n";                    
        }
    }
...
/**
 * @brief       Writes all shape's perimeters
 *
 * @detailed    As the loop increments, it will call the
 *              write function and then write the shape's perimeter until the 
 *              end of the list
**/

void ShapeList::print_perimeters() const{
...
/**
 * @brief       Writes all shape's areas
 *
 * @detailed    As the loop increments, it will call the
 *              write function and then write the shape's area until the end
 *              of the list
**/

void ShapeList::print_areas() const {
    for (const auto& shape : *this){       // for each `Shape*` in the list
        shape->write(cout);             
